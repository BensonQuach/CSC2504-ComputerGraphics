...
public:
    ...
    // Reder parameter setters.
    void set_glossy_reflections(bool set) { _glossy_reflections = set; }
...
private:
  ...
  // Computes the reflected direction with respect to a surfaces
  // unit normal and a incident vector.
  Vector3D reflect( Vector3D& incident, Vector3D& normal );
    
  // Computes the refracted direction vector t with respect to a surfaces
  // unit normal, incident vector, coming from index of refraction n1 and
  // entering index of refraction n2.
  bool refract( Vector3D& incident, Vector3D& normal, double n1, double n2, Vector3D* t );

	// Return the colour of the ray after intersection and shading, call 
	// this function recursively for reflection and refraction.  
	Colour shadeRay( Ray3D& ray, int depth, int maxDepth ); 
  ...
  // Returns true iff the ray does not intersect any object in the scene between
  // its origin and the given light source.
  bool isRayVisible( Ray3D& ray, LightSource* light );
